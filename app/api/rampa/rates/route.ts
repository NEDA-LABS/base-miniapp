/**
 * Explicit route for GET /api/rampa/rates - proxies to backend.
...
export async function GET() {
    ...
    console.log('[Rampa Proxy] >>> GET /api/rampa/rates (request received)');

    try {
        if (!NEDAPAY_API_KEY) {
            console.error('[Rampa Proxy] rates: API key missing');
            return NextResponse.json({ error: 'API key missing' }, { status: 500 });
        }

        const targetUrl = `${NEDAPAY_API_BASE}/api/v1/ramp/rampa/rates`;
        ...
        if (status >= 400) {
            console.error(`[Rampa Proxy] rates → ${response.status} in ${elapsed}ms`, text.slice(0, 150));
        } else {
            const preview = typeof data === 'object' && data !== null ? JSON.stringify(data).slice(0, 100) : '';
            console.log(`[Rampa Proxy] rates → ${status} in ${elapsed}ms`, preview);
        }
        return NextResponse.json(data, { status });
    } catch (error) {
        const elapsed = Date.now() - start;
        console.error(`[Rampa Proxy] rates error in ${elapsed}ms:`, error);
